	...
	if (niClObject && niClObject->body.get())
	{
		...
		if (rigidBody && rigidBody->referencedObject.get())
		{
			...
			if (havokRigidBody)
			{
				...
				if (clObject.CollisionObject->sceneObject)
				{
...
bool Util::GetBBCharacter(RE::TESObjectREFR* object, ObjectBound& bound)
{
	...
	RE::NiPoint3 niPosition;
	RE::NiMatrix3 niRotation;

	auto lifeState = characterObject->AsActorState()->GetLifeState();
	if (lifeState == RE::ACTOR_LIFE_STATE::kDead || lifeState == RE::ACTOR_LIFE_STATE::kDying)
	{
		...
		niPosition = spine->world.translate;
		niRotation = spine->world.rotate;
	}
	else
	{
		niPosition = object->GetPosition();
		niRotation = object->GetAngle();

		niPosition.z += characterObject->GetHeight() / 2.0f;
	}

	auto collisionBound = characterObject->GetCharController()->collisionBound.extents;

	bound.boundMin = glm::vec3(-collisionBound.x, -collisionBound.y, -collisionBound.z);
	bound.boundMax = glm::vec3(collisionBound.x, collisionBound.y, collisionBound.z);

	bound.rotation = RotMatrixToEuler(niRotation);

	//auto niPosition = spine->world.translate;
...
std::vector<CollisionFocusObject> Util::FindCollisionObjectListByOverride(RE::NiNode* nodeIn, ObjectOverride objOverride)
{
    std::vector<CollisionFocusObject> clObjects;

	if (!nodeIn)
        return clObjects;

	for (auto& currClOverride : objOverride.ClObjects)
	{
        RE::NiNode* targetNode = nodeIn;
		for (int i = 0; i < currClOverride.Path.size(); i++)
		{
            std::string currPathName = currClOverride.Path[i];

            targetNode = GetChildNodeByName(targetNode, currPathName.c_str());
            if (!targetNode)
                break;
		}
		...
		if (targetNode)
		{
            ...
            newClObject.SetData(targetNode->collisionObject.get(), targetNode->parent);
            newClObject.UpdateBound();

			clObjects.push_back(newClObject);
		}
	}
...
void Util::ForEachReferenceInRange(RE::NiPoint3 originPos, float a_radius, std::function<RE::BSContainer::ForEachResult(RE::TESObjectREFR* a_ref)> a_callback)
{
	...
	if (a_radius > 0.0f)
	{
		if (tesSingleton->interiorCell)
		{
			tesSingleton->interiorCell->ForEachReferenceInRange(originPos, a_radius, [&](RE::TESObjectREFR* a_ref)
			{
		...
		/*if (const auto skyCell = tesSingleton->worldSpace ? tesSingleton->worldSpace->GetSkyCell() : nullptr; skyCell)
		{
...
void Util::CellForEachReferenceInRange(RE::TESObjectCELL * cell, const RE::NiPoint3 & a_origin, float a_radius, std::function<RE::BSContainer::ForEachResult(RE::TESObjectREFR*)> a_callback)
{
...
bool Util::IsPlayerInFirstPerson()
{
    auto playerCam = RE::PlayerCamera::GetSingleton();
    if (!playerCam)
        return false;

    return playerCam->currentState->id == RE::CameraState::kFirstPerson ||
           (Compatibility::IFPV::IsEnabled && Compatibility::IFPV::IsFirstPersonGlobal && Compatibility::IFPV::IsFirstPersonGlobal->value);
}

uint32_t * Util::GetControlsEnabledPtr()
{
	auto post1130 = REL::Module::get().IsAE() && (REL::Module::get().version().patch() > 0x400);
	auto enConAddr = (std::uintptr_t)RE::ControlMap::GetSingleton() + std::ptrdiff_t(post1130 ? 0x120 : 0x118);

	return REL::Relocation<std::uint32_t(*)>(enConAddr).get();
}

void Util::DisableControlFlag(RE::ControlMap::UEFlag flag)
{
	auto* controlsEnabled = GetControlsEnabledPtr();

	if (controlsEnabled)
		*controlsEnabled &= ~((uint32_t)flag);
}

void Util::EnableControlFlag(RE::ControlMap::UEFlag flag)
{
	auto* controlsEnabled = GetControlsEnabledPtr();
