...
void DebugAPI::DrawLine2D(RE::GPtr<RE::GFxMovieView> movie, glm::vec2 from, glm::vec2 to, float color, float lineThickness, float alpha)
{
	...
	AccurateClampToScreen(from);
	AccurateClampToScreen(to);
...
// two points and scale the vector accordingly -> AccurateClampToScreen
...
void DebugAPI::AccurateClampToScreen(glm::vec2& point)
{
	glm::vec2 screenRes(ScreenResX, ScreenResY);

	if (point.x < 0 || point.x > screenRes.x ||
		point.y < 0 || point.y > screenRes.y)
	{
		// center location within allowed rect (accounting for text bounds)
		glm::vec2 allowedCenter = screenRes * 0.5f;

		// direction from center to posIn
		glm::vec2 centerToPos = point - allowedCenter;
		glm::vec2 dirToPos = glm::normalize(centerToPos);

		float tX = (dirToPos.x > 0) 
            ? (screenRes.x - allowedCenter.x) / dirToPos.x 
            : (0 - allowedCenter.x) / dirToPos.x;
		float tY = (dirToPos.y > 0) 
			? (screenRes.y - allowedCenter.y) / dirToPos.y 
			: (0 - allowedCenter.y) / dirToPos.y;

		float moveMagnitude = std::min(tX, tY);

		float overshoot = point.length() - moveMagnitude;
		if (overshoot > CLAMP_MAX_OVERSHOOT)
			...
			point = allowedCenter + dirToPos * moveMagnitude;
	}
}
...
glm::vec2 DebugAPI::WorldToScreenLoc(RE::GPtr<RE::GFxMovieView> movie, glm::vec3 worldLoc)
{
	...
	RE::NiCamera::WorldPtToScreenPt3((float(*)[4])Offsets::WorldToCamMatrix.address(), *(RE::NiRect<float>*)Offsets::ViewPort.address(), niWorldLoc, screenLocOut.x, screenLocOut.y, zVal, 1e-5f);
